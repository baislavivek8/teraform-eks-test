resource "aws_wafv2_web_acl" "main" {
  ...
  name  = "skilrock-waf-global-eks-${var.environment}"
  scope = "CLOUDFRONT"

  description =   "skilrock-waf-global-eks-${var.environment}"
    ...
    content {
      ...
      dynamic "visibility_config" {
        ...
        content {
          ...
          metric_name                = lookup(visibility_config.value, "metric_name", "skilrock-waf-global-eks-default-rule-metric-name")
  ...
  tags = {
    Product     = "skilrock-waf-global-eks"
  ...
  dynamic "visibility_config" {
    ...
    content {
      cloudwatch_metrics_enabled = lookup(visibility_config.value, "cloudwatch_metrics_enabled", true)
      metric_name                = lookup(visibility_config.value, "metric_name", "skilrock-waf-global-eks-default-web-acl-metric-name")
      sampled_requests_enabled   = lookup(visibility_config.value, "sampled_requests_enabled", true)
    }
  }
}
