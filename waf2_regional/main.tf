resource "aws_wafv2_web_acl" "main" {
  ...
  name  = "skilrock-waf-regional-eks-${var.environment}"
  scope = "REGIONAL"

  description = "skilrock-waf-regional-eks-${var.environment}"
  default_action {
    dynamic "allow" {
      for_each = true ? [1] : []
      content {}
    ...
    content {
      ...
      dynamic "visibility_config" {
        ...
        content {
          ...
          metric_name                = lookup(visibility_config.value, "metric_name", "skilrock-waf-Regional-eks-default-rule-metric-name")
  ...
  tags = {
    Product     = "skilrock-waf-regional-eks"
  ...
  dynamic "visibility_config" {
    ...
    content {
      cloudwatch_metrics_enabled = lookup(visibility_config.value, "cloudwatch_metrics_enabled", true)
      metric_name                = lookup(visibility_config.value, "metric_name", "skilrock-waf-regional-eks-default-web-acl-metric-name")
      sampled_requests_enabled   = lookup(visibility_config.value, "sampled_requests_enabled", true)
    }
  }
}
